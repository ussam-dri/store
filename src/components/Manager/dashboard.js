...
const ManagerDashboard = () => {
  ...
  const filteredMembers = [ ...sellers].filter(member => {
        ...
        <div className="flex-1 p-6">
          ...
          <div className="hidden md:block">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-2xl font-bold">Sellers list</h2>
                <p className="text-gray-600">See information about all Products and Sellers</p>
              </div>
              <div className="space-x-2">
                <button className="px-4 py-2 bg-blue-600 text-white rounded-md flex items-center">
                  <UserPlusIcon className="h-5 w-5 mr-2" />
                  <a href='/manager/addSeller'>ADD MEMBER</a>
                </button>
              </div>
            </div>
              ...
              <tbody className="text-gray-600 text-sm font-light">
                {filteredMembers.map((member) => (
                  <tr key={member._id} className="border-b border-gray-200 hover:bg-gray-100">
                      ...
                      <div className="flex items-center">
                        <div className="mr-2">
                          <img className="w-10 h-10 rounded-full"
                            src={`https://backend-mern-store.zelobrix.comdownload/${member.photo.filename}`}
                            alt={member.name} />
                        </div>
                        <div>
                    ...
                    <td className="py-3 px-6 text-left">
                      <a href={`/manager/editSeller/${member.role}/${member._id}`} className="text-gray-500 hover:text-gray-700">
